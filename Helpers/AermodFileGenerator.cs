...
namespace GeoProj.Helpers
{
    ...
    {
        public static void GenerateInputFiles(List<AermodSource> sources, ReceptorSettings receptorSettings, string baseDirectoryPath, List<BuildingFootprint> buildings = null)
        {
            ...
            GenerateAermodInput(aermodDir, sources, receptorSettings, buildings);
        }
        ...
        private static void GenerateAermodInput(string aermodDir, List<AermodSource> sources, ReceptorSettings receptorSettings, List<BuildingFootprint> buildings = null)
        {
            ...
            if (buildings != null && buildings.Count > 0)
            {
                ...
                foreach (var b in buildings)
                {
                    ...
                    // Associate buildings with all sources (for downwash effects)
                    foreach (var source in sources)
                    {
                        sb.AppendLine(string.Format(ci,
                            "   BUILDHGT {0} {1:F1}", source.SourceId, b.HeightMeters));
                    }
